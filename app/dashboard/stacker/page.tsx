...
export default function StackerPage() {
  ...
  const update=(id:number,key:keyof Account,val:any)=>setAccounts(prev=>prev.map(a=>a.id===id?{...a,[key]:val}:a))
  ...
  const inp:React.CSSProperties={background:'#F7F8FA',border:'1.5px solid #E8EAF0',borderRadius:8,padding:'7px 10px',fontSize:12,color:'#1A1D2E',outline:'none',fontFamily:"'Nunito Sans',sans-serif",width:'100%'}
    ...
    <div>
      ...
      <Card style={{marginBottom:16}}>
        ...
        <div style={{overflowX:'auto'}}>
          <div style={{display:'grid',gridTemplateColumns:'140px 110px 90px 100px 100px 90px 70px 36px',gap:8,padding:'0 0 10px',borderBottom:'1px solid #F0F2F5',marginBottom:4}}>
          ...
          {accounts.length===0?(
            <div style={{textAlign:'center',padding:'24px 0',color:'#9EA6C0',fontSize:13}}>No accounts. Click "+ Add account" to start.</div>
          ):accounts.map(a=>{
            const monthly=a.size*a.daily/100*a.split/100*a.days
            return (
              <div key={a.id} style={{display:'grid',gridTemplateColumns:'140px 110px 90px 100px 100px 90px 70px 36px',gap:8,padding:'8px 0',borderBottom:'1px solid #F5F7FA',alignItems:'center'}}>
                <select value={a.firm} onChange={e=>update(a.id,'firm',e.target.value)} style={sel}>{FIRMS.map(f=><option key={f}>{f}</option>)}</select>
                ...
                <select value={a.status} onChange={e=>update(a.id,'status',e.target.value)} style={{...sel,color:statusColor(a.status),fontWeight:700,fontSize:11}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        ...
        <Card>
          ...
          {totalMonthly>=50000&&<Alert type="success">You're projecting $50K+/month. Incredible — stay consistent!</Alert>}
          {totalMonthly>0&&totalMonthly<10000&&<Alert type="info">Add more funded accounts to reach $10K/month faster.</Alert>}
          {totalMonthly===0&&<Alert type="info">Add your funded accounts above to see income projections.</Alert>}
        </Card>
      </div>
    </div>
  )
